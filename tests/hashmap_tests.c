...
Test(map_suite, 02_multithreaded, .timeout = 2, .init = map_init, .fini = map_fini) {
    ...
    *val_ptr = 55;
    ...
    if(pthread_create(&thread_ids[22], NULL, thread_put, insert) != 0){
        exit(EXIT_FAILURE);
    }

    pthread_join(thread_ids[22], NULL);

    int* search_key_base = malloc(sizeof(int));
    *search_key_base=22;
    map_key_t search_key;
    search_key.key_base = search_key_base;
    search_key.key_len = sizeof(int);

    //printf("GET: %d\n",*((int*)(get(global_map,search_key).val_base)));
    cr_assert_eq(55, *((int*)(get(global_map,search_key).val_base)), "Failed to replace value at 22");
